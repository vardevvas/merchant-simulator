package com.goal.merchantsimulator.Validation;

import com.goal.merchantsimulator.dto.inbound.BillInquiryRequest;
import com.goal.merchantsimulator.dto.inbound.BillPaymentRequest;
import org.springframework.stereotype.Component;

import java.util.Map;
import java.util.function.Function;

@Component
public interface BillPaymentValidation extends Function<BillPaymentRequest, Map<Object,Object >> {

    default BillPaymentValidation and(BillPaymentValidation other) {
        return request -> {
            Map<Object,Object > result = this.apply(request);
            return result.containsValue(0) ? other.apply(request) : result;
        };
    }
}
